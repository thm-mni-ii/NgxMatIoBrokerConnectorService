...
export class NgxMatIoBrokerConnectorService {
  ...
  states: any;            // Stores all states that exist, returned from the socket adapter
  servConn: any;
  ...
  registerGetter(object: string, state: string) {
    ...
    this.getters.map((x, index) => {
      if (x.id === object + '.' + state) {
  ...
  // Removes a registered getter by removing it from the getters array
  removeRegistered(object: string, state: string) {
    this.getters.map((x, index) => {
      if (x.id === object + '.' + state) {
        this.getters.splice(index, 1);
      }
    })
  }
